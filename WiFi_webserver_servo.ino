...
void saveSensorDataToCSV(AsyncWebServerRequest *request);
...
void setup() {
  ...
  // Route for saving sensor data to CSV
  server.on("/save-to-csv", HTTP_POST, saveSensorDataToCSV);
...
// Function to handle saving sensor data to CSV file
void saveSensorDataToCSV(AsyncWebServerRequest *request) {
  // Retrieve sensor data from POST request
  String sensor1_current = request->getParam("sensor1_current")->value();
  String sensor2_current = request->getParam("sensor2_current")->value();
  String voltage = request->getParam("voltage")->value();
  String dateTime = request->getParam("dateTime")->value();

  // Open CSV file in append mode
  File csvFile = SPIFFS.open("C:\\Users\\sawan\\OneDrive\\Documents\\Arduino\\WiFi_webserver_servo\\data\\sensor_data.csv", FILE_APPEND);
  if (!csvFile) {
  ...
  csvFile.print(dateTime);
  csvFile.print(",");
  csvFile.print(sensor1_current);
  csvFile.print(",");
  csvFile.print(sensor2_current);
  csvFile.print(",");
  csvFile.println(voltage);
  csvFile.close();

  request->send(200, "text/plain", "Sensor data saved to CSV");
}
